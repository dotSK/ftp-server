...
// CONSTANTS
#define PORT_NUM 4785
#define NUM_OF_WORKERS 10
#define EPOLL_MAX_EVENTS 100
#define EPOLL_TIMEOUT 100

// GLOBAL VARIABLES
static bool server_end = false;
static StrBuf_t *cwd;

// FUNCTIONS
int main(int argc, char *argv[]);
static bool init_server(int sock_fd);
static bool conn_arbiter(int sock_fd, pthread_t *thread_ids,
                         ThrdData *thrd_data, size_t thrd_data_len);
...
int main(int argc, char *argv[static argc + 1]) {
    ...
    // TODO: GNUism
    cwd = StrBuf_FromCharPtr(get_current_dir_name());

    // print some information
    fprintf(stdout, "listening on port: %d\n", PORT_NUM);
    fprintf(stdout, "current working directory: %s", StrBuf_getCharPtr(cwd));
...
static bool init_server(int sock_fd) {
    ...
    // TODO: LINUXism
    const uint32_t cpu_count = (uint32_t)sysconf(_SC_NPROCESSORS_ONLN);
    // TODO: get rid of upper limit
    // TODO: is assert OK here?
    assert(cpu_count > 0 && cpu_count < 1024);
    thread_count = (cpu_count - 1) + NUM_OF_WORKERS;
    ...
    for (int i = 0; i < thread_count; i++) {
        if (pthread_create(thread_ids + i, NULL, &server_thrd_start,
                           &thrd_data) != 0) {
            free(thread_ids);
            return false;
        }
        ...
        if (result >= 0) {
            ...
            switch (errno) {
                ...
                case EMFILE || ENFILE || ENOBUFS || ENOMEM:
                    puts("WARN: Not enough resources to accept connection, "
                         "dropping...");
                    break;
...
static void *server_thrd_start(const ThrdData *data) {
    int epoll_fd = 0;
    ...
    while (!server_end) {
        ...
        if (epoll_status == -1) {
            ...
            // TODO: check worker threads? What are we doing when no new
            // connection arrives?
        }
        ...
        if (epoll_wait != 1) {
            ...
            switch (event_queue[0].events) {
                ...
                case EPOLLIN:
                    // FIXME: lolwut?
                    if (((ConnState *)event_queue[0].data.ptr)->fd ==
                        data->sock_fd) {
                        // accept connection
                ...
                default:
                    fputs("epoll_wait returned non-selected signal, this "
                          "should not happen",
                          stderr);
                    server_end = 1;
                    break;
    ...
    while (!end_of_session) {
        ...
        if (!strcmp(recv_buf, "user")) {
            if (!strcmp(cmd_argument, "anonymous")) {
                ...
                send_status(530,
                            "This FTP server supports only anonymous login");
            }
        } else {
            send_status(530, "Please login with USER and PASS");
        }
...
void start_session(int sock_fd, char *recv_buf, int buf_size) {
    ...
    if (path_buf == NULL) {
        fputs("failed to allocate memory for ftp temp directory buffer",
              stderr);
        pthread_exit(NULL);
    }
    path_buf_size = 100;
        ...
        if (!strcmp(*recv_buf, "acct"))
            ...
            if (*cmd_argument != '\0') {
                ...
                if ((temp = get_bound_sock(0, INADDR_ANY)) > 0) {
                    ...
                    getsockname(temp, (struct sockaddr *)&temp_addr,
                                &temp_addr_len);
            ...
            if (*cmd_argument == '\0') {
                ...
                if ((temp1 = parse_ip_port(cmd_argument, &remote_ip)) > 0) {
                    ...
                    if ((temp = get_sock(temp1, remote_ip)) > 0) {
                        if (pthread_mutex_trylock(&worker_data->work_mtx) !=
                            0) {
                            pthread_kill(worker_data->t_info, SIGUSR1);
                            pthread_mutex_lock(&worker_data->work_mtx);
                        }
                        worker_data->sock_fd = temp;
                        memset(&worker_data->remote, 0,
                               sizeof(worker_data->remote));
            ...
            } else if (!strcmp(cmd_argument, "i") ||
                       !strcmp(cmd_argument, "l 8") ||
                       !strcmp(cmd_argument, "I") ||
                       !strcmp(cmd_argument, "L 8")) {
        ...
        switch (worker_data->data_op) {
                ...
                if (worker_data->conn_defined) {
                    if (!worker_data->pasv) {
                        if ((connect(worker_data->sock_fd,
                            ...
                            fputs("active socket connection to client failed",
                                  stderr);
                            break;
                        } else {
                    ...
                    while (strlen(worker_data->dirinfo.c_dir) > path_buf_size) {
                        buf_temp =
                            realloc(path_buf, 2 * path_buf_size * sizeof(char));
                        if (buf_temp == NULL) {
                            fputs("cannot allocate more memory to ftp data "
                                  "worker path buffer, "
                                  "skipping operation",
                                  stderr);
                            close(sock_fd);
                    ...
                    fputs("ftp thread requested action without making a "
                          "connection first",
                          stderr);
                    worker_data->trans_ok = 0;
                }
                ...
                if (worker_data->conn_defined) {
                    ...
                    if (worker_data->b_flag) {
                        if ((fp = fopen(worker_data->fjel, "rb")) != NULL) {
                            ...
                            while (!((temp = fread(data_buf, sizeof(char),
                                                   data_buf_size, fp)) <= 0) ||
                                   !feof(fp)) {
                                temp1 =
                                    ftp_send_binary(sock_fd, data_buf, temp);
                                if (temp1 == -1) {
                            ...
                            fputs(
                                "file from worker data does not exist, lolwut?",
                                stderr);
                        }
                    } else {
                        if ((fp = fopen(worker_data->fjel, "r")) != NULL) {
                            if (worker_data->start_pos != 0)
                                fseek(fp, worker_data->start_pos, SEEK_SET);
                            while ((buf_temp = fgets(data_buf, data_buf_size,
                                                     fp)) != NULL &&
                                   !(feof(fp) || ferror(fp))) {
            ...
            case STOR:
                ...
                if (worker_data->file_w != NULL) {
                    while ((temp = recv(sock_fd, data_buf, data_buf_size, 0)) >
                           0) {
                        if (fwrite(data_buf, sizeof(*data_buf), temp,
                                   worker_data->file_w) != temp) {
